*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZVCONF_DIR
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZVCONF_DIR          .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZVCONF_DIR'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZVCONF_DIR_EXTRACT
TOTAL = ZVCONF_DIR_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZVCONF_DIR'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZVCONF_DIR-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVCONF_DIR_EXTRACT
TOTAL = ZVCONF_DIR_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZVCONF_DIR-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZVCONF_DIR'
...
TOTAL = ZVCONF_DIR_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZVCONF_DIR-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZVCONF_DIR-UPD_FLAG NE SPACE.
